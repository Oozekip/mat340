...
pub fn run_gamblers(){
    ...
    let start = read_until_checked("Enter amount of money to start with", check_in_range(0, goal));
...
pub fn run_polya(){
    let green = read_until_checked("Enter the number of green balls to start with", check_at_least(1));
    let orange = read_until_checked("Enter the number of orange balls to start with", check_at_least(1));
    let additional = read_until_checked("Enter the number of additional balls to add", check_at_least(1));
...
pub fn polya_urn(green: u32, orange: u32, additional: u32, n: u32) -> Vec<f64>{
    ...
    for _ in 0..n{
        ...
        let index = ((curr_green as f32) / ((curr_green + curr_orange) as f32) * 100.0) as usize / 5;

        results[index] += 1;
    }

    results.iter().map(|x| f64::from(*x) / f64::from(n)).collect()
}
